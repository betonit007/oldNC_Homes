...
export default function HouseForm({ }: IProps) {
    ...
    useEffect(() => {
        register({ name: "address" }, { required: "Please enter  your house" })
    ...
    return (
        <form className='mx-auto max-w-xl py-4' onSubmit={handleSubmit(onSubmit)}>
            ...
            { address && <>
                <div className="mt-4">
                    <label htmlFor="image" className='p-4 border-4 border-gray-600 block cursor-pointer'>
                    ...
                    <input
                        ...
                        onChange={(event: ChangeEvent<HTMLInputElement>) => {
                            if (event?.target?.files?.[0]) {
                                ...
                                reader.readAsDataURL(file) //convert image to string, a string can be passed to img url
                            }
                        }}
                    />
                    {previewImage &&
